...
class TFRecordDataset:
    def __init__(self,
        ...
        # self.dtype              = 'uint8'
        self.dtype = 'float32'
        self.dynamic_range = [-50.0, 50.0]  # [0.0, 255.0]
        ...
        with tf.name_scope('Dataset'), tf.device('/cpu:0'):
            ...
            self._tf_init_ops = {lod: self._tf_iterator.make_initializer(dset) for lod, dset in
                                 self._tf_datasets.items()}
...
class SyntheticDataset:
    def __init__(self, resolution=1024, num_channels=3, dtype='float32', dynamic_range=[-50.0, 50.0], label_size=0,
                 label_dtype='float32'):
    ...
    def _generate_images(self, minibatch, lod,
                         shape):  # to be overridden by subclasses # pylint: disable=unused-argument
...
def load_dataset(class_name='training.dataset.TFRecordDataset', data_dir=None, verbose=False, **kwargs):
    ...
    if 'tfrecord_dir' in adjusted_kwargs and data_dir is not None:
        ...
        print("load dataset: ", adjusted_kwargs['tfrecord_dir'])
    if verbose:
        print('Streaming data using %s...' % class_name)
    dataset = dnnlib.util.get_obj_by_name(class_name)(**adjusted_kwargs)  # <class 'training.dataset.TFRecordDataset'>
    if verbose:
