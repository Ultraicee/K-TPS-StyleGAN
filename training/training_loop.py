...
def training_schedule(
    ...
    lod_training_kimg       = 140,      # Thousands of real images to show before doubling the resolution.
    lod_transition_kimg     = 140,      # Thousands of real images to show when fading in new layers.
    ...
    tick_kimg_base          = 100,      # Default interval of progress snapshots.160
    tick_kimg_dict          = {4: 100, 8:80, 16:70, 32:50, 64:30, 128:20, 256:10, 512:10, 1024:10}): # Resolution-specific overrides.
    #4: 160, 8:140, 16:120, 32:100, 64:80, 128:60, 256:40, 512:30, 1024:20
...
def training_loop(
    ...
    minibatch_repeats       = 4,        # Number of minibatches to run before adjusting training parameters.##4
    reset_opt_for_new_lod   = True,     # Reset optimizer internal state (e.g. Adam moments) when new layers are introduced?
    total_kimg              = 2500,    # Total length of the training, measured in thousands of real images.
    ...
    for gpu in range(submit_config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, training_set=training_set, minibatch_size=minibatch_split, **G_loss_args)##training.loss.G_logistic_nonsaturating
            with tf.name_scope('D_loss'), tf.control_dependencies(lod_assign_ops):
                D_loss = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set, minibatch_size=minibatch_split, reals=reals, labels=labels, **D_loss_args)##training.loss.D_logistic_simplegp
    ...
    print('Training...\n')
    ctx.update('', cur_epoch=resume_kimg, max_epoch=total_kimg)#max_epoch是total_kimg
