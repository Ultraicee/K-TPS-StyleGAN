...
def G_style(
        ...
        truncation_cutoff=6,  # Number of layers for which to apply the truncation trick. None = disable.
...
# ----------------------------------------------------------------------------
# Mapping network used in the K-StyleGAN paper.
...
def G_mapping(
        ...
        normalize_latents=False,  # Normalize latent vectors (Z) before feeding them to the mapping layers?True
...
def G_synthesis(
        dlatents_in,  # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].W，512维
        dlatent_size=16,  # Disentangled latent (W) dimensionality.
        num_channels=3,  # Number of output color channels.
        resolution=1024,  # Output resolution.分辨率
        ...
        structure='auto',
        # 'fixed' = no progressive growing, 'linear' = human-readable, 'recursive' = efficient, 'auto' = select
        # automatically.
        is_template_graph=False,  # True = template graph constructed by the Network class, False = actual evaluation.
    ...
    with tf.variable_scope('4x4'):
        if const_input_layer:
            ...
            with tf.variable_scope('Dense'):
                x = dense(dlatents_in[:, 0], fmaps=nf(1) * 16, gain=gain / 4,
    ...
    if structure == 'recursive':
        ...
        def grow(x, res, lod):
            ...
            img = cset(img, (lod_in > lod),
                       lambda: upscale2d(tflib.lerp(torgb(res, y), upscale2d(torgb(res - 1, x)), lod_in - lod),
                                         2 ** lod))
...
def D_basic(
    ...
    act, gain = {'relu': (tf.nn.relu, np.sqrt(2)), 'lrelu': (leaky_relu, np.sqrt(2))}[nonlinearity]  # lrelu
    ...
    if structure == 'recursive':
        ...
        def grow(res, lod):
            ...
            if res > 2: y = cset(y, (lod_in > lod),
                                 lambda: tflib.lerp(x, fromrgb(downscale2d(images_in, 2 ** (lod + 1)), res - 1),
                                                    lod_in - lod))
            return y()
